...
int IntersectionTests::objectBoxOverlap(double normal[3],double d, double maxbox[3])
{
  int q;
...
int IntersectionTests::objectBoxOverlap(float normal[3],float d, float maxbox[3])
{
  int q;
  float vmin[3],vmax[3];
  for(q=X;q<=Z;q++)
...
int IntersectionTests::triBoxOverlap(double boxcenter[3],double boxhalfsize[3],double triverts[3][3])
{
  ...
  // double axis[3];
   ...
   /* Bullet 2: */
   /*  test if the box intersects the object of the triangle */
   /*  compute object equation of triangle: normal*x+d=0 */
   CROSS(normal,e0,e1);
   d=-DOT(normal,v0);  /* object eq: normal.x+d=0 */
   if(!objectBoxOverlap(normal,d,boxhalfsize)) return 0;
